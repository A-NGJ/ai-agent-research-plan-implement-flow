...
- **Pipeline-mode split**: compute a complexity score for the design — `score = max(0, components-1) + max(0, dirs-1) + (multi_spec ? 2 : 0)`, where `components` counts `### N. ...` subheadings under `## Components` (fall back to `## File Structure`), `dirs` counts distinct top-level directories in `## File Structure` (root files like CLAUDE.md count as one bucket), and `multi_spec` is true when the design references >1 file under `.rpi/specs/`. If `score ≥ 4`, propose a labeled breakdown of sibling plans before writing any plan: each plan covers ≥1 design component, the union covers every component, slugs follow `<design-topic>-<scope-slug>`, and dependencies form a DAG. Run accept / edit / single-plan with the user; under `--ff`, accept the auto-generated proposal. On cycle detection, surface the participating plans and re-enter the edit loop without writing files. Splits are pipeline-only.
